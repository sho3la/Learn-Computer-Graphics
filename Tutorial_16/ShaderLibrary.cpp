...
ShaderLibrary::ShaderLibrary()
{
	...
	fs = R"CODE(
			...
			void main()
			{
				vec4 pixel_color = vec4(1,1,1,texture(atlas,vertex_uv).r);
				
				if(user_color == vec4(0,0,0,0)) finalcolor = vertex_color * pixel_color;
				else finalcolor = vertex_color * pixel_color * user_color;
			};
)CODE";

	programs.insert(pair<string,ShaderProgram*>("Font",new ShaderProgram(vs,fs)));
}
